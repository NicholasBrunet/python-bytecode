import multiprocessing as mp
import os

ALLOWED_CORE_TIME_MILLIES: int = int(os.environ.get("ALLOWED_CORE_TIME_MILLIES", 100000))
CORE_COUNT: float = ALLOWED_CORE_TIME_MILLIES / 100000

UNIX_SOCKET_PATH: str = os.environ.get("UNIX_SOCKET_PATH", "")
SOCKET_TIMEOUT: float = 1.0

# SERVER

DEFAULT_LOGGING_LEVEL: int = 2 # 0: off, 1: orchestrator, 2: workers
DEFAULT_WORKER_CLOCK_TICK_SPEED: int = 0.1
DEFAULT_ORCHESTRATOR_CLOCK_TICK_SPEED: int = 1
DEFAULT_WORKER_CORES: int = 1
DEFAULT_RUNTIMES_PER_WORKER_CORE: int = 5
MAX_WORKER_CORES: int = CORE_COUNT - 1
MAX_RUNTIMES_PER_WORKER_CORE: int = 10
MAX_LOGS: int = 1000